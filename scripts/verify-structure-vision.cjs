...
const observedScan = scanStructure({ moleculeName: "benzene", visualAnalysis: observedAnalysis, manualHints: { moleculeName: "benzene" } })
assert.equal(observedScan.bestMatch?.record.id, "benzene", "combined scan prefers benzene")
assert.ok(observedScan.bestMatch.confidence >= 90 && observedScan.bestMatch.confidence <= 97, "explicit hint plus visual agreement reaches high fusion confidence")
