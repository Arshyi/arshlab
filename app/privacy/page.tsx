...
const sections = [
  ...
  },
  {
    ...
    body: "The v6.3.0 Structure Scanner processes uploaded images and accepted camera snapshots locally in the browser. Scene understanding, semantic region labels, molecule segmentation, molecule crops, arrow detection, text-region separation, border suppression, reflection masks, human-object masks, perspective-normalized canvas detection, selected quadrilaterals, deskewed crops, glare-reduced variants, candidate-region generation, bond-length scoring, multi-crop fallback, image variants, OCR, global shape reconstruction, reconstructed strokes, polygon hypotheses, candidate graph generation, global graph optimization, consensus graph solving, legal graph repair history, calibrated confidence, canonical graph hashes, ring-closure recovery, chemical graph validation, edge pruning, valence diagnostics, molecular graphs, evidence fusion, overlays, and optional overlay PNG export remain local. Original images, rejected crops, selected crops, normalized crops, scene graphs, semantic region boxes, OCR text, atom positions, reconstructed stroke data, graph hypotheses, consensus graph histories, optimizer moves, consensus repair moves, ring polygons, bridge events, graph geometry, rejected-bond diagnostics, and engine votes are not uploaded to ARSHLAB servers, Supabase, OpenRouter, or an external AI API. Images are not permanently stored or included in local scan history. Local history stores only match labels, source labels, confidence, functional-group labels, timestamps, and corrections.",
  },
  {
